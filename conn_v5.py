                ...
                if ser.in_waiting > 0:
                    ...
                    if role == "admin":
                        if room_status == "Unoccupied":
                            ...
                            print(f"[Room {self.room_id}] Admin exited → All OFF")
                        continue

                    # --- FACULTY EXIT LOGIC ---
                    if role == "faculty" and room_status == "Occupied":
                        # Faculty can exit manually (turn everything off)
                        self.log_access(user_id, uid, schedule_id, 'Exit', 'granted')
                        self.db.execute("UPDATE classrooms SET Status = 'Unoccupied' WHERE Room_id = %s", (self.room_id,))
                        self.send_command(ser, 'ROOM_OFF')
                        print(f"[Room {self.room_id}] Faculty exit detected → Room OFF")
                        continue
