...
class AutoOffMonitor(threading.Thread):
    ...
    def run(self):
        print("🕒 AutoOffMonitor started...")
        while True:
            now = datetime.now().strftime("%H:%M:%S")
            day = datetime.now().strftime("%a")

            # Find rooms with schedules that have ended but still marked occupied
...
class RFIDReader(threading.Thread):
    ...
    def send_command(self, ser, cmd):
        ...
        try:
            ...
            print(f"[Room {self.room_id}] ⚠️ Send failed: {e}")

    def log_access(self, user_id, uid, schedule_id, access_type, status):
        """Record every RFID access attempt."""
        self.db.execute("""
    ...
    def run(self):
        """Main RFID reader loop."""
        try:
            ser = serial.Serial(self.port, 9600, timeout=1)
            time.sleep(2)
            print(f"📡 RFID Reader active on {self.port} (Room {self.room_id})")
            ...
            while self.running:
                if ser.in_waiting > 0:
                    ...
                    # Ignore empty or non-UID startup messages
                    if not uid or uid.upper() == "READY" or not any(c.isalnum() for c in uid):
                        continue

                    # Debounce duplicate scans
                    if uid == self.last_uid and now - self.last_scan_time < 3:
                        continue


                    self.last_uid, self.last_scan_time = uid, now
                    print(f"[Room {self.room_id}] UID: {uid}")
                    ...
                    if not user_id:
                        ...
                        print(f"[Room {self.room_id}] ❌ Access Denied → {reason}")
                        continue
                    ...
                    room_status = self.db.query("SELECT Status FROM classrooms WHERE Room_id=%s",
                                                (self.room_id,), fetchone=True)["Status"]

                    # 🧑‍💼 ADMIN: Toggle room ON/OFF
                    if role == "admin":
                        ...
                        self.send_command(ser, f"ROOM_TOGGLE:{new_status.upper()}")
                        print(f"[Room {self.room_id}] Admin toggled → {new_status}")
                        continue

                    # 👩‍🏫 FACULTY: Turn room ON/OFF depending on occupancy
                    if role == "faculty" and status == "granted":
                        if room_status == 'Unoccupied':
                            ...
                            self.send_command(ser, "ROOM_ON:ALL")
                            print(f"[Room {self.room_id}] Faculty entered → all ON")
                        else:
                            self.db.execute("UPDATE classrooms SET Status='Unoccupied' WHERE Room_id=%s", (self.room_id,))
                            self.send_command(ser, "ROOM_OFF")
                            print(f"[Room {self.room_id}] Faculty exited → all OFF")
                        self.log_access(user_id, uid, sched_id, 'Entry/Exit', 'granted')
                        continue

                    # 👨‍🎓 STUDENT: Light control only
                    if role == "student" and status == "granted":
                        if room_status == 'Unoccupied':
                            self.db.execute("UPDATE classrooms SET Status='Occupied' WHERE Room_id=%s", (self.room_id,))
                            self.send_command(ser, "ROOM_ON:LIGHT")
                            print(f"[Room {self.room_id}] Student entered → lights ON")
                        else:
                            self.db.execute("UPDATE classrooms SET Status='Unoccupied' WHERE Room_id=%s", (self.room_id,))
                            self.send_command(ser, "ROOM_OFF")
                            print(f"[Room {self.room_id}] Student exited → lights OFF")
                        self.log_access(user_id, uid, sched_id, 'Entry/Exit', 'granted')
                        continue

                    # 🚫 Access denied (inactive or out of schedule)
                    self.send_command(ser, "DENIED")
                    self.log_access(user_id, uid, sched_id, 'Entry', 'denied')
                    print(f"[Room {self.room_id}] ❌ Access Denied → {reason}")

        except Exception as e:
            print(f"[Room {self.room_id}] ⚠️ Reader error: {e}")
